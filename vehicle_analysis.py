...
class AppConfig:
    ...
    plate_crop_padding_ratio: float = 0.08
    plate_resize_scale: float = 2.0
    display_window_name: str = "Vehicle ANPR Analytics"
    font_scale: float = 0.6
    line_thickness: int = 2
    box_thickness: int = 2
    track_ttl_sec: float = 3.0
    stable_plate_hits_required: int = 2
    plate_match_history_size: int = 8
    plate_text_stability_threshold: float = 0.65
        ...
        while not self.stop_event.is_set():
            if self.cap is None or not self.cap.isOpened():
                if not self._open_capture():
                    ...
                    sleep_for = min(backoff, self.reconnect_backoff_max_sec)
                    LOGGER.warning(f"Reconnecting in {sleep_for:.1f}s (attempt {reconnect_attempts})")
                    time.sleep(sleep_for)
                    if reconnect_attempts >= self.max_reconnect_attempts_before_backoff:
                        backoff = min(backoff * 1.5, self.reconnect_backoff_max_sec)
                    continue
                reconnect_attempts = 0
                backoff = self.reconnect_delay_sec
                ...
                if not ok or frame is None:
                    if self.is_file_source and self.loop_video:
                        try:
                            ...
                            LOGGER.warning(f"Video loop reset failed: {e}")
                            break
...
class OCRWorker(threading.Thread):
    ...
    def _process_crop(self, crop: np.ndarray) -> np.ndarray:
        try:
            ...
            return cv2.resize(denoised, (int(w * 2.0), int(h * 2.0)), interpolation=cv2.INTER_CUBIC)
        except Exception as e:
    ...
    def run(self) -> None:
        while not self.stop_event.is_set():
            try:
                ...
                continue
            try:
                pre = self._process_crop(job.plate_crop)
...
class VehicleAnalysisEngine:
    ...
    def _prune_tracks(self) -> None:
        try:
            ...
            LOGGER.exception(f"Track prune error: {e}")
            ...
            try:
                with self.track_lock:
                    ...
                    if result.clean_text:
                        ...
                        if candidate and vote_ratio >= self.cfg.plate_text_stability_threshold and avg_conf >= self.cfg.ocr_min_confidence:
                            ...
                            if state.stable_plate_hits >= self.cfg.stable_plate_hits_required and not state.logged:
                                ...
                                LOGGER.info(f"Logged track_id={state.track_id}, plate={state.confirmed_plate}")
            except Exception as e:
    ...
    def _crop_with_padding(self, frame: np.ndarray, bbox: Tuple[int, int, int, int], pad_ratio: float) -> np.ndarray:
        try:
            ...
            LOGGER.exception(f"Crop error: {e}")
                        ...
                        for box in boxes:
                            try:
                                ...
                                if best_plate_bbox is not None:
                                    ...
                                    px1 = clamp(px1, 0, vehicle_crop.shape[1] - 1)
                                    py1 = clamp(py1, 0, vehicle_crop.shape[0] - 1)
                                    px2 = clamp(px2, 0, vehicle_crop.shape[1] - 1)
                                    py2 = clamp(py2, 0, vehicle_crop.shape[0] - 1)
                                ...
                                cv2.putText(display_frame, label, (x1, max(20, y1 - 10)), cv2.FONT_HERSHEY_SIMPLEX,
                                            self.cfg.font_scale, color, self.cfg.line_thickness)
                            except Exception as e:
